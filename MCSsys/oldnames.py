        ...
        for histvar in self.histvarnames:
            ...
            for x in self.histstatenames:
               ...
               if self.histstatedesc[kkk]=="LiH MC":
                  ...
                  # print "MC"
                  # print hists
               elif self.histstatedesc[kkk]=="ref MC":
                  fMC = TFile("MC"+self.fname)
                  x = 'ref'
                  hists.append(fMC.Get(histvar+'_'+x))
               else:
            ...
            for n in self.histstatenames:
                for x in self.normnames:
                 ...
                 if self.histstatedesc[jjj]=="ref MC":
                        fMCnorm = TFile("MC"+self.fname)
                        norms.append(fMCnorm.Get(x)[0]*3)
                        jjj += 1
                        # print "sdgbsfgbsfgbsfbgs"
            ...
            elseif "LiH" in self.histstatedesc[i]:
                t1 = TText(0.18,0.885,"MICE Internal")
                t4 = TText(0.18,0.85,"ISIS cycle 2015/04")
                t2 = TText(0.18,0.815,"LiH, "+self.desc[1][2:5]+", MAUS v3.3.2")
                t3 = TText(0.18,0.84,"Central, "+str((self.central/self.total)*100)+",%")
            elseif "No Absorber" in self.histstatedesc[i]:
                t1 = TText(0.18,0.885,"MICE Internal")
                t4 = TText(0.18,0.85,"ISIS cycle 2015/04")
                t2 = TText(0.18,0.815,"LiH, "+self.desc[1][2:5]+", MAUS v3.3.2")
                t3 = TText(0.18,0.84,"Central, "+str((self.central/self.total)*100)+",%")
            ...
            t4.SetTextSize(0.03)
            t4.SetTextFont(42)
            hists[0].GetYaxis().SetRangeUser(4e-5,0.04)
            if self.histstatedesc[1]=="ref MC":
                hists[0].GetYaxis().SetRangeUser(4e-5,0.08)
            ...
            for h in hists:
                ...
                if histvar == 'theta2Scatt':
                   ...
                   par =  g.GetParameters()
                   t5 = TText(0.18,0.8,"m="+str("%.2f" % par[0]))
                   t5.SetNDC(1)
                   t5.SetTextSize(0.04)
                   t5.SetTextFont(42)
                   t5.Draw()
...
if __name__ == '__main__':
    ...
    raw172.histstatedesc = ['ref Data','ref MC']
    raw200.histstatenames = ['ref','refMC']
    raw200.histstatedesc = ['ref Data','ref MC']
    raw240.histstatenames = ['ref','refMC']
    raw240.histstatedesc = ['ref Data','ref MC']
