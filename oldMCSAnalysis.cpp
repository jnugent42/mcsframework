...
//    pStart_vec_y(TVectorD(19)), theta_true_x(TVectorD(19)), theta_true_y(TVectorD(19))
{
  ...
  cor_mom = new TH1D("cor_mom","Cor Momentum Calculated from TOF; Momentum (MeV/c)", 180, 120, 300);
  mccalc_mom = new TH1D("mccalc_mom","Momentum Calculated from TOF; Momentum (MeV/c)", 130, 120, 250);
  ...
  theta_true_x_graph =
    new TH1D("thetaX_graph","Change in Projected Angle (X);#Delta#theta_{X}; Events",_histlimits["NbinsXY"], _histlimits["minXY"], _histlimits["maxXY"]);
  theta_true_y_graph =
    new TH1D("thetaY_graph","Change in Projected Angle (X);#Delta#theta_{X}; Events",_histlimits["NbinsXY"], _histlimits["minXY"], _histlimits["maxXY"]);
  minustheta_true_x_graph =
    new TH1D("minusthetaX_graph","Change in Projected Angle (X);#Delta#theta_{X}; Events",_histlimits["NbinsXY"], _histlimits["minXY"], _histlimits["maxXY"]);
  minustheta_true_y_graph =
    new TH1D("minusthetaY_graph","Change in Projected Angle (X);#Delta#theta_{X}; Events",_histlimits["NbinsXY"], _histlimits["minXY"], _histlimits["maxXY"]);
  ...
  mctheta_true_x_graph =
    new TH1D("thetaX_graphmc","Change in Projected Angle (X);#Delta#theta_{X}; Events",_histlimits["NbinsXY"], _histlimits["minXY"], _histlimits["maxXY"]);
  mctheta_true_y_graph =
    new TH1D("thetaY_graphmc","Change in Projected Angle (X);#Delta#theta_{X}; Events",_histlimits["NbinsXY"], _histlimits["minXY"], _histlimits["maxXY"]);
...
int no=0;
  ...
  Collection MCUSAllTOF, MCDSAllTOF, MCUSPreRadSel, MCDSPreRadSel;
  double pz = 0.;
  ...
  for (int i=0; i<1000000; i++){
    ...
    FillCollectionSciFi(USAllTOF, jUS, kUS, lUS, pz, 0);
    FillCollectionSciFi(DSAllTOF, jDS, kDS, lDS, pz, 1);
    ...
    MCUSAllTOF.append_instance(mcUSTruthAbsHit);
    MCDSAllTOF.append_instance(mcDSTruthAbsHit);
    }


    if ( !PIDSelection(true) ) continue;
    //if ( !TruthTime12(true) ) continue; // event_ok=false;
    if (mode==-2) { 
    ...
    if (USrefplaneI > 0 && USabsPlaneI >0 &&
      DSrefplaneI > 0 && DSabsPlaneI >0 ){
    MCUSPreRadSel.append_instance(mcUSTruthAbsHit);
    MCDSPreRadSel.append_instance(mcDSTruthAbsHit);
    }
    if ( !RadialSelection(pz,19948.8,meanp) ) continue;
    cuts_accept->Fill("Fiducial Selection",1);
    if ( !RadialSelection(pz,_sys["diffpos"],90) ) continue; //diff=1;
    ...
    Nevents++;
    //
    if (!FillCrossCheck()) continue;
    cuts_accept->Fill("chi2ndf",1);
    if(_sys["Nevents"] > 0 && Nevents > int(_sys["Nevents"]))
      break;

    //if (scifievent->scifitracks()[jUS]->scifitrackpoints()[kUS]->pos().x()<30 && scifievent->scifitracks()[jUS]->scifitrackpoints()[kUS]->pos().y()<30 && scifievent->scifitracks()[jUS]->scifitrackpoints()[kUS]->pos().x()>-30 && scifievent->scifitracks()[jUS]->scifitrackpoints()[kUS]->pos().y()>-30) { 
  /*  
    double MCTruth_pz_mid; 
   for ( size_t iter=0; iter < mcevent->GetVirtualHits()->size(); iter++){
	   if (mcevent->GetVirtualHits()->at(iter).GetPosition().z()-17000<50 && mcevent->GetVirtualHits()->at(iter).GetPosition().z()-17000>-50) {
		   MCTruth_pz_mid = mcevent->GetVirtualHits()->at(iter).GetMomentum().z();
	   }
   }
   if (MCTruth_pz_mid>202 && MCTruth_pz_mid<198) continue; 
   */
    FillCollectionSciFi(_USset, jUS, kUS, lUS, pz, 0);
    FillCollectionSciFi(_DSset, jDS, kDS, lDS, pz, 1);
    FillCollectionSciFi(_UStmpset, jUS, kUS, lUS, pz, 0);
    //mcUSTruthAbsHit.pz = pz;
    if (USrefplaneI > 0 && USabsPlaneI >0 &&
      DSrefplaneI > 0 && DSabsPlaneI >0 ){
    mcreconUSTruthSet.append_instance(mcUSTruthAbsHit);
    mcreconDSTruthSet.append_instance(mcDSTruthAbsHit);
    }
  ...
  data_make_beam_histograms(USPreRadSel, "Upstream, Data", "dataUS_prerad", MCUSPreRadSel);
  data_make_beam_histograms(DSPreRadSel, "Downstream, Data", "dataDS_prerad", MCDSPreRadSel);
...
void MCSAnalysis::referenceSelection(){
  ...
  for (int i=0; i<1000000; i++){
    ...
    cuts_accept->Fill("US Track Found",1);
    FillCollectionSciFi(USAllTOFEmpty, jUS, kUS, lUS, pz, 0);
    FillCollectionSciFi(DSAllTOFEmpty, jDS, kDS, lDS, pz, 1);
    Vars mcUSTruthAbsHitEmpty, mcDSTruthAbsHitEmpty;
    int centre = 51;
    if (USrefplaneI > 0 && USabsPlaneI >0 &&
      DSrefplaneI > 0 && DSabsPlaneI >0 ){
    FillVarsVirtual(mcUSTruthAbsHitEmpty, centre, centre);
    FillVarsVirtual(mcDSTruthAbsHitEmpty, centre, centre);
    ...
    //if ( !TruthTime12(false) ) continue; // event_ok=false;
    cuts_accept->Fill("TOF Selection",1);
    ...
    // if (scifievent->scifitracks().size() != 2) continue;
    // if ( !SelectMomentum() ) continue;
    FillCollectionSciFi(USPreRadSelEmpty, jUS, kUS, lUS, pz, 0);
    FillCollectionSciFi(DSPreRadSelEmpty, jDS, kDS, lDS, pz, 1);
  if (USrefplaneI > 0 && USabsPlaneI >0 &&
      DSrefplaneI > 0 && DSabsPlaneI >0 ){
    ...
    if ( !RadialSelection(pz,19948.8,meanp) ) continue;
    cuts_accept->Fill("Fiducial Selection",1);
    if ( !RadialSelection(pz,_sys["diffpos"],90) ) continue;
    cuts_accept->Fill("Diffuser Cut",1);
   ...
   */
    if (!FillCrossCheck()) continue;
    cuts_accept->Fill("chi2ndf",1);
    FillCollectionSciFi(_USMCset, jUS, kUS, lUS, pz, 0);
    FillCollectionSciFi(_DSMCset, jDS, kDS, lDS, pz, 1);
  if (USrefplaneI > 0 && USabsPlaneI >0 &&
      DSrefplaneI > 0 && DSabsPlaneI >0 ){
...
//	  std::cout << ngood << std::endl;
    ...
    if ( !TruthRadialSelection(pz,_sys["diffpos"],90,USrefplaneI) ) continue;
...
bool MCSAnalysis::MatchUSDS(){
  ...
  if( scifievent->scifitracks().size() == 1 || 
	...
	if(tracker==0 && zpos > maxUS){
	  maxUS = zpos;
	  kUS = k;
		//std::cout << "kUS" << scifievent->scifitracks()[j]->scifitrackpoints()[k]->pos().z() << std::endl;
	  jUS = j;
	}
	if(tracker==0 && zpos < minDS){
		//std::cout << "lUS" << scifievent->scifitracks()[j]->scifitrackpoints()[k]->pos().y() << std::endl;
	  lUS = k;
	}
	if(tracker==1 && zpos < minDS){      
	  minDS = zpos;
	  kDS = k;
		//std::cout << "kDS" << scifievent->scifitracks()[j]->scifitrackpoints()[k]->pos().z() << std::endl;
	  jDS = j;
	}
	if(tracker==1 && zpos > maxUS){      
		//std::cout << "lDS" << scifievent->scifitracks()[j]->scifitrackpoints()[k]->pos().y() << std::endl;
	  lDS = k;
	}
      }
...
bool MCSAnalysis::FillCrossCheck(){
...
bool MCSAnalysis::PIDSelection(bool isdata=true){
  ...
  */
  ...
  if(isdata){
    ...
    if ( rawTOF2HitTime - rawTOF1HitTime < TOF_lower_limit+0.02 ||
	 rawTOF2HitTime - rawTOF1HitTime > TOF_upper_limit+0.02) return false;
    // if ( rawTOF2HitTime - rawTOF1HitTime < TOF_lower_limit ||
    //      rawTOF2HitTime - rawTOF1HitTime > TOF_upper_limit) return false;
...
bool MCSAnalysis::TruthTime12(bool isdata=true){
  ...
  if ((tof2-tof1) > TOF_upper_limit+0.02 || (tof2-tof1) < TOF_lower_limit+0.02) return false; 
  }
  return true;
}
...
bool MCSAnalysis::RadialSelection(double pz, double pos, double radius){
  ...
  if (jUS == -1 || kUS == -1) 
    ...
    USplane.dXdz = scifievent->scifitracks()[jUS]->scifitrackpoints()[kUS]->mom().x()/USplane.pz +
      tan(_sys["thXUS"] * atan(1.)/45.0);
    USplane.dYdz = scifievent->scifitracks()[jUS]->scifitrackpoints()[kUS]->mom().y()/USplane.pz +
...
std::vector<double> MCSAnalysis::mcRotDefineProjectionAngles(Vars US, Vars DS,int i){
...
double MCSAnalysis::PathLengthInLH2(double pz){
    ...
    DSplane.dXdz = scifievent->scifitracks()[jDS]->scifitrackpoints()[kDS]->mom().x()/DSplane.pz +
      tan(_sys["thXDS"] * atan(1.)/45.0);
    DSplane.dYdz = scifievent->scifitracks()[jDS]->scifitrackpoints()[kDS]->mom().y()/DSplane.pz +
   ...
   */
   ...
   for ( size_t j=0; j < mcevent->GetVirtualHits()->size(); j++){
	   ...
	   if (mcevent->GetVirtualHits()->at(j).GetPosition().z()-17000<50 && mcevent->GetVirtualHits()->at(j).GetPosition().z()-17000>-50) {
		   MCTruth_pz_mid = mcevent->GetVirtualHits()->at(68).GetMomentum().z();
	   }
   ...
   pcor = 105.65/sqrt(pow(t_ds,2)/pow(dt0,2)-1);
   ...
   if (mat==1) {
	   //mccalc_mom->Fill(pcor);
   }
   TOF12vsMCTruth->Fill(pcor, MCTruth_pz_mid);
   double res12 = pcor - MCTruth_pz_mid;
...
// sp, gap = gap after and before distribution for deconvolution
  ...
  float* source_theta2Scatt = new float[sp+gap+int(_histlimits["NbinsTh2"])];

  for(int i=0;i<_DSMCset.N(); i++){
    std::vector<double> projTheta = RotDefineProjectionAngles(_USMCset.E(i), _DSMCset.E(i), angdef);
    //double thetaMCY = atan(_DSMCset.E(i).dXdz) - atan(_USMCset.E(i).dXdz);
    //double thetaMCX = atan(_DSMCset.E(i).dYdz) - atan(_USMCset.E(i).dYdz);
  ...
  //thetaX_response = trkreffix(thetaX_response);
  //thetaY_response = trkreffiy(thetaY_response);
  ...
  float* response_theta2Scatt = new float[sp+gap+int(_histlimits["NbinsTh2"])];
  ...
  for(int i=0; i<_histlimits["NbinsXY"]; i++){
    ...
    response_thetaX[i+1] = thetaX_response->GetBinContent(i+1);
    response_thetaY[i+1] = thetaY_response->GetBinContent(i+1);
  }
  for(int i=0; i<_histlimits["NbinsTh"]; i++){
    source_thetaScatt[2*sp+i] = thetaScatt_recoGold->GetBinContent(i+1);
    response_thetaScatt[i+1] = thetaScatt_response->GetBinContent(i+1);
  }
  for(int i=0; i<_histlimits["NbinsTh2"]; i++){
    source_theta2Scatt[sp+i] = theta2Scatt_recoGold->GetBinContent(i+1);
    response_theta2Scatt[i+1] = theta2Scatt_response->GetBinContent(i+1);
  }
...
// source_* changed each iteration so Source_* is left unchanged
  ...
  //sX->Deconvolution(source_thetaX, response_thetaX, sp+gap+int(_histlimits["NbinsXY"]), 20, 10, 0.0);

  TSpectrum *sY = new TSpectrum();
  //sY->Deconvolution(source_thetaY, response_thetaY, sp+gap+int(_histlimits["NbinsXY"]), 20, 10, 0.0);
  /*
...
// test results added to arrays
  ...
  for(int i=0; i<int(_histlimits["NbinsXY"]); i++){
    thetaX_recoGold->SetBinContent(i+1, source_thetaX[sp+i]);
    thetaY_recoGold->SetBinContent(i+1, source_thetaY[sp+i]);
  }
/*
  for(int i=0; i<int(_histlimits["NbinsXY"])i/2; i++){
    thetaX_recoGold->SetBinContent(i+1, source_thetaX[sp+int(_histlimits["NbinsXY"])i-i]);
    thetaY_recoGold->SetBinContent(i+1, source_thetaY[sp+int(_histlimits["NbinsXY"])i-i]);
  }
  for(int i=23; i<int(_histlimits["NbinsXY"]); i++){
    thetaX_recoGold->SetBinContent(i+1, source_thetaX[sp+i]);
    thetaY_recoGold->SetBinContent(i+1, source_thetaY[sp+i]);
  }
  */
...
void MCSAnalysis::DoVirtualDeconvolution(std::string model, int n_sel=1){
  ...
  tmpname += model;
  TH1* thetaX_asymm = 
    new TH1D(tmpname.c_str(),"Change in Projected Angle (X);#Delta#theta_{XY}; Events",_histlimits["NbinsXY"]/2, 0, _histlimits["maxXY"]);
  ...
  tmpname += model;
  TH1* thetaY_asymm = 
    new TH1D(tmpname.c_str(),"Change in Projected Angle (Y);#Delta#theta_{Y}; Events",_histlimits["NbinsXY"]/2, 0, _histlimits["maxXY"]);
  ...
  for (int l=0;l<180;l++){
      ...
      tmpname += l;
      Histo[l] = defineHist2(tmpname.c_str(),tmpname.c_str(),46, -0.1, 0.1);
...
void MCSAnalysis::FillCollectionSciFi(Collection& Set, int j, int k, int l, double pz, int isDS, bool project){      

  if(j < int(scifievent->scifitracks().size()) && j != -1){
    if(k < int(scifievent->scifitracks()[j]->scifitrackpoints().size()) && k != -1){
      Vars tmpvar;
      FillVarsSciFi(tmpvar, j, k, l, pz, isDS);
      /*
      if (project){
	      std::cout << "2" << std::endl;
...
void MCSAnalysis::FillVarsSciFi(Vars& tmpvar, int j, int k, int l, double pz, int isDS){
    ...
    if(k < int(scifievent->scifitracks()[j]->scifitrackpoints().size()) && k != -1){
      ...
      if (isMC==1){
	      tmpvar.dXdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x() + tan(thX*atan(1.)/45.0);
              tmpvar.dYdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().y() + tan(thY*atan(1.)/45.0);
	      /* 
	      //std::cout << "p dYdz " << tmpvar.dYdz << std::endl;
	      if(isDS==0){
	      tmpvar.dXdz = (scifievent->scifitracks()[j]->scifitrackpoints()[l]->pos().x()-scifievent->scifitracks()[j]->scifitrackpoints()[k]->pos().x())/_sys["trkrlength"] + tan(thX*atan(1.)/45.0);
	      tmpvar.dYdz = (scifievent->scifitracks()[j]->scifitrackpoints()[l]->pos().y()-scifievent->scifitracks()[j]->scifitrackpoints()[k]->pos().y())/_sys["trkrlength"] + tan(thX*atan(1.)/45.0);
	      //std::cout << "grad dYdz " << tmpvar.dYdz << std::endl;
	      } else { 
	      tmpvar.dXdz = (scifievent->scifitracks()[j]->scifitrackpoints()[k]->pos().x()-scifievent->scifitracks()[j]->scifitrackpoints()[l]->pos().x())/_sys["trkrlength"] + tan(thX*atan(1.)/45.0);
	      tmpvar.dYdz = (scifievent->scifitracks()[j]->scifitrackpoints()[k]->pos().y()-scifievent->scifitracks()[j]->scifitrackpoints()[l]->pos().y())/_sys["trkrlength"] + tan(thX*atan(1.)/45.0);
	      }
	      */
      }
      else {
      if(isEmpty){
      if(isDS==0){
	      tmpvar.dXdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().x()/tmpvar.pz*cos(rot_ang_empty)+sin(rot_ang_empty) + tan(thX*atan(1.)/45.0);
      } else { 
	      tmpvar.dXdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().x()/tmpvar.pz + tan(thX*atan(1.)/45.0);
      }
      // scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().z();
      if(isDS==0){
	      tmpvar.dYdz = (scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().y()/tmpvar.pz*cos(rot_ang_empty))-sin(rot_ang_empty) + tan(thY*atan(1.)/45.0);
      } else { 
	      tmpvar.dYdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().y()/tmpvar.pz + tan(thY*atan(1.)/45.0);
      }
      }
      else {
      if(isDS==0){
	      //tmpvar.dXdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x()*cos(rot_ang)+scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().y()*-sin(rot_ang) + tan(thX*atan(1.)/45.0);
	      tmpvar.dXdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().x()/tmpvar.pz*cos(rot_ang) + sin(rot_ang)+tan(thX*atan(1.)/45.0);
	      double ori = scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().x()/tmpvar.pz + tan(thX*atan(1.)/45.0);
	      double justcos = scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().x()/tmpvar.pz*cos(rot_ang) + tan(thX*atan(1.)/45.0);
	      double justsin = scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().x()/tmpvar.pz*cos(rot_ang)+sin(rot_ang) + tan(thX*atan(1.)/45.0);
	      double full = scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().x()/tmpvar.pz*cos(rot_ang)+scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().z()/tmpvar.pz*sin(rot_ang) + tan(thX*atan(1.)/45.0);
              /*	      
	      std::cout << "rot_ang " << rot_ang << std::endl;
	      std::cout << "ori " << ori << std::endl;
	      std::cout << "justcos " << justcos << std::endl;
	      std::cout << "justsin " << justsin << std::endl;
      ...
      // scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().z();
      if(isDS==0){
	      //tmpvar.dYdz = (scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x()*sin(rot_ang))+scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().y()*cos(rot_ang) + tan(thY*atan(1.)/45.0);
	      tmpvar.dYdz = (scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().y()/tmpvar.pz*cos(rot_ang))-sin(rot_ang) + tan(thY*atan(1.)/45.0);
      } else { 
	      tmpvar.dYdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->mom().y()/tmpvar.pz + tan(thY*atan(1.)/45.0);
      }
      }
