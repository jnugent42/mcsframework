...
#include "reduce_tof1_trigger.hh"
...
int main(int argc, char* argv[]) {
	...
	Spill->GetEntry(i);
	  ...
	  for (size_t ii = 0;  ii < spill->GetReconEvents()->size(); ++ii) {
		  ...
		  if (tof_event != NULL)
			  MAUS::TOFEventSpacePoint sp = tof_event->GetTOFEventSpacePoint();
		  else
			  continue;
		  ...
		  bool scifi = false;
		  bool radial = false;
		  bool radialdif = false;
		  if ( tof_event->GetTOFEventSpacePoint().GetTOF0SpacePointArraySize() >= 1 ) {
			  sp0 = true;
		  }
		  if ( tof_event->GetTOFEventSpacePoint().GetTOF1SpacePointArraySize() >= 1 ) {
			  sp1 = true;
		  }
		  if( (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks().size() == 1 || (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks().size() == 2){ 
			  scifi = true;
		  }
		  /*
		  int jUS=-1;
		  int kUS=-1;
		  if( (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks().size() == 1 || (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks().size() == 2){
			  for ( size_t j=0; j<(*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks().size(); j++){
				  int npoints = (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[j]->scifitrackpoints().size();
				  double maxUS=0.0, minDS=44000;
				  int tracker = (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[j]->tracker();
				  for ( int k=0; k<npoints; k++){
					  double zpos = 
						  (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[j]->scifitrackpoints()[k]->pos().z();
		  ...
		  if (jUS == -1 || kUS == -1) {
			  ...
			  Vars USplane;
			  USplane.X = (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[jUS]->scifitrackpoints()[kUS]->pos().x();
			  USplane.Y = (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[jUS]->scifitrackpoints()[kUS]->pos().y();
			  USplane.Z = (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[jUS]->scifitrackpoints()[kUS]->pos().z();
			  USplane.dXdz = (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[jUS]->scifitrackpoints()[kUS]->gradient().x();
			  USplane.dYdz = (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[jUS]->scifitrackpoints()[kUS]->gradient().y();
			  USplane.pz   = 300; 
			  USplane.px   = (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[jUS]->scifitrackpoints()[kUS]->mom().x();
			  USplane.py   = (*spill->GetReconEvents())[ii]->GetSciFiEvent()->scifitracks()[jUS]->scifitrackpoints()[kUS]->mom().y();
			  double phi = atan2(USplane.dYdz, USplane.dXdz);
			  double zdiff = 2*fabs(USplane.Z - pos);
			  Vars DSproj;
			     DSproj = PropagateVarsMu(USplane, posdif);
			     if ( sqrt(DSproj.X*DSproj.X + DSproj.Y*DSproj.Y) < radiusdif) {
			     radialdif = true;
			     }
			     DSproj = PropagateVarsMu(USplane, abspos);
			     DSproj.dXdz += sigmap*cos(phi);
			     DSproj.dYdz += sigmap*sin(phi);
			     DSproj.px   += sigmap*cos(phi)*DSproj.pz;
			     DSproj.py   += sigmap*sin(phi)*DSproj.pz;
			     DSproj = PropagateVarsMu(USplane, pos);
			     if ( sqrt(DSproj.X*DSproj.X + DSproj.Y*DSproj.Y) < radius) {
			     radial = true;
			     }

			     }
			     */
			  //if( sp0 && sp1 && scifi && radial && radialdif){
			  if( sp0 && sp1 && scifi){
				  TOF1count++;
				  tofevent   = tof_event;
	  ...
	  if(type.find("sim")!=std::string::npos){
	    for (size_t ij=0; ij<spill->GetMCEvents()->size(); ij++){
		    ...
		    for (int jj=0; jj<(*spill->GetMCEvents())[ij]->GetVirtualHitsSize();jj++) {
			    int station_id = (*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetStationId();
			    if (station_id == 65){
				    mcscifi = true;
				    /*
				    Vars USplane;
				    USplane.X = (*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetPosition().x();
				    USplane.Y = (*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetPosition().y();
				    USplane.Z = (*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetPosition().z();
				    USplane.dXdz = (*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetMomentum().x()/(*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetMomentum().z();
				    USplane.dYdz = (*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetMomentum().y()/(*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetMomentum().z();
				    USplane.pz   = (*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetMomentum().z();
				    USplane.px   = (*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetMomentum().x();
				    USplane.py   = (*spill->GetMCEvents())[ij]->GetVirtualHits()->at(jj).GetMomentum().y();
				    Vars DSproj = PropagateVarsMu(USplane, posdif);
				    if ( sqrt(DSproj.X*DSproj.X + DSproj.Y*DSproj.Y) < radiusdif) selecteddif = true;
				    double phi = atan2(USplane.dYdz, USplane.dXdz);
				    double zdiff = 2*fabs(USplane.Z - pos);
				    DSproj = PropagateVarsMu(USplane, abspos);
				    DSproj.dXdz += sigmap*cos(phi);
				    ...
				    DSproj.px   += sigmap*cos(phi)*DSproj.pz;
				    DSproj.py   += sigmap*sin(phi)*DSproj.pz;
				    DSproj = PropagateVarsMu(DSproj, pos);
				    if ( sqrt(DSproj.X*DSproj.X + DSproj.Y*DSproj.Y) < radius) selected = true;
				    */
			    }
		    }
		    
		    for (int i=0;i<tofhit->size();i++) {
			    //if (tofhit->at(i).GetPosition().Z() - 12929 < 20 and tofhit->at(i).GetPosition().Z() - 12929 > -20 and mcscifi == true and selected == true and selecteddif == true){        
			    if (tofhit->at(i).GetPosition().Z() - 12929 < 20 and tofhit->at(i).GetPosition().Z() - 12929 > -20 and mcscifi == true){        
