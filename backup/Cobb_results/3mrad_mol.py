from ROOT import TH1D, TFile, TCanvas, TLegend, TGraph, TROOT
import ROOT
...
def MoliereDist(datafile, outfile):
    ...
    ofile.cd();
    ROOT.gDirectory.Delete("thetaScatt_Moliere;4")
    ROOT.gDirectory.Delete("thetaX_Moliere;4")
    ROOT.gDirectory.Delete("thetaY_Moliere;4")
    ROOT.gDirectory.Delete("thetaScatt_Moliere;3")
    ROOT.gDirectory.Delete("thetaX_Moliere;3")
    ROOT.gDirectory.Delete("thetaY_Moliere;3")
    ROOT.gDirectory.Delete("thetaScatt_Moliere;2")
    ROOT.gDirectory.Delete("thetaX_Moliere;2")
    ROOT.gDirectory.Delete("thetaY_Moliere;2")
    # ROOT.gDirectory.Delete("thetaScatt_Moliere;2")
    # ROOT.gDirectory.Delete("thetaX_Moliere;2")
    # ROOT.gDirectory.Delete("thetaY_Moliere;2")
    sdir = '/data/neutrino03/jnugent/Unfolding/backup/Cobb_results/moliere'
    ...
    for l in fin:
        ln = l.split()
        data.append([i, float(ln[0]), float(ln[1]), float(ln[0]) - last, float(ln[1])+lasttheta])
        last = float(ln[0])
        lasttheta = float(ln[1])
        i+=1
    ...
    datahalf = []
    xprjbin = array('d')
    for x in range(0,798,2):
        # print x,data[x][1],data[x][3],data[x][1]-data[x][3]/2
        datahalf.append([m, data[x][1], data[x][2]+data[x+1][2],data[x][3]])
        m += 1
        xprjbin.append(data[x][1] - data[x][3]/2.)
    ...
    hthetaX = TH1D("thetaX_Moliere",";#theta_{X}; Events per 2 mrad",len(xprjbin)-1,xprjbin)
    hthetaY = TH1D("thetaY_Moliere",";#theta_{Y}; Events per 2 mrad",len(xprjbin)-1,xprjbin)

    for d in range(0,399,1):
        # print datahalf[d][0]+1,datahalf[d][1],datahalf[d][2]
        hthetaX.SetBinContent(datahalf[d][0]+1, datahalf[d][2])
        hthetaY.SetBinContent(datahalf[d][0]+1, datahalf[d][2])
...
def PickUpGEANTModel(datafile, outfile):
   ...
   '''
   modfile.cd();
   ROOT.gDirectory.Delete("thetaScatt_Moliere;1")
   ROOT.gDirectory.Delete("thetaX_Moliere;1")
   ROOT.gDirectory.Delete("thetaY_Moliere;1")
   '''

   modfile.cd()
   ROOT.gDirectory.Delete("thetaScatt_GEANT;1")
   ROOT.gDirectory.Delete("thetaX_GEANT;1")
   ROOT.gDirectory.Delete("thetaY_GEANT;1")
   MCFile.cd()
   histx = MCFile.Get("thetaX_graph")
   histy = MCFile.Get("thetaY_graph")
   histscatt = MCFile.Get("theta_true_scat_graph")
...
def binno(oldfile,newfile):

   oldfile = TFile(oldfile, "UPDATE")
   newfile = TFile(newfile, "READ")

   oldfile.cd();
   histx = oldfile.Get("thetaX_GEANT")
   print 'old',histx.GetNbinsX()

   newfile.cd()
   histx = newfile.Get("thetaX_GEANT")
   print 'new',histx.GetNbinsX()
...
if __name__ == '__main__':
    ...
    #MoliereDist('LiH-6p5-172-Molprj-fine.dat','LiH-6p5-172-B-hists.root')
    MoliereDist('LiH-6p5-200-Molprj-fine.dat','LiH-6p5-200-B-hists.root')
    #MoliereDist('LiH-6p5-240-Molprj-fine.dat','LiH-6p5-240-hists.root')
    # PickUpGEANTModel('LiH-172-MCTruth.root','LiH-6p5-172-B-hists.root')
    # PickUpGEANTModel('LiH-200-MCTruth.root','LiH-6p5-200-B-hists.root')
    # PickUpGEANTModel('LiH-240-MCTruth.root','LiH-6p5-240-hists.root')
    #binno('oldfile.root','newfile.root')
