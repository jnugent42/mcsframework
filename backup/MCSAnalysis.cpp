    ...
    //    pStart_vec_y(TVectorD(19)), theta_true_x(TVectorD(19)), theta_true_y(TVectorD(19))
{
    ...
    TOFvsMCTruth = new TH2D("TOFvsMCTruth","TOFvsMCTruth", 33, 100, 350,33, 100, 350);
    refTOFvsMCTruth = new TH2D("refTOFvsMCTruth","refTOFvsMCTruth", 33, 100, 350,33, 100, 350);
    ...
    uncor_mom = new TH1D("uncor_mom","Cor Momentum Calculated from TOF; Momentum (MeV/c)", 180, 120, 300);
    uncor_momsel = new TH1D("uncor_momsel","Cor Momentum Calculated from TOF; Momentum (MeV/c)", 180, 120, 300);
    mccalc_mom = new TH1D("mccalc_mom","Momentum Calculated from TOF; Momentum (MeV/c)", 100, 0, 400);
...
void MCSAnalysis::Write(){
    ...
    c1->Clear();
    TOFvsMCTruth->GetXaxis()->SetTitle("pz TOF MeV/c");
    TOFvsMCTruth->GetYaxis()->SetTitle("pz MC Truth at abs. centre (MeV/c)");
    ...
    c1->Clear();
    refTOFvsMCTruth->GetXaxis()->SetTitle("pz TOF (MeV/c)");
    refTOFvsMCTruth->GetYaxis()->SetTitle("pz MC Truth at abs. centre (MeV/c)");
...
bool MCSAnalysis::PIDSelection(const bool& isdata=true){
    ...
    double MCSAnalysis::CorMomFromTOF(Vars& set, double& diff, const bool& isdata){
        ...
        if (material=="LiH") {
            ...
            if (!isdata) {
                ...
                /* pcor = sqrt(pow(Efin,2)-pow(105,2)) + 2.15; */
                /* pUS = sqrt(pow(Efin,2)-pow(105,2)) + 2.15; */
            }
        }
        /* if (mat==1) { */
        ...
        for ( size_t j=0; j < mcevent->GetVirtualHits()->size(); j++){
            ...
            if (mcevent->GetVirtualHits()->at(j).GetPosition().z()-16950<50 && mcevent->GetVirtualHits()->at(j).GetPosition().z()-16950>-50) {
        ...
        if (t_ds != 10000) {
            ...
            pDS = 105.65/sqrt(pow(t_ds,2)/pow(dte12,2)-1) - refDScor;
                /* std::cout << "pcor " << pcor << std::endl; */
                /* pcor = 105.65/sqrt(pow(t_ds,2)/pow(dte12,2)-1) - 1.8; */
            /* pDS = 105.65/sqrt(pow(t_ds,2)/pow(dte12,2)-1) - 1.8; */
                std::cout << "pcor " << pcor << std::endl;
            }
    ...
    double MCSAnalysis::getoffset(xmlNode * a_node, bool file){
        ...
        refDScor = strtod(frefDScor, &pEnd);
        std::cout << "refDScor " << refDScor << std::endl;
        }
        if (file==1){
        UScor = strtod(fDScor, &pEnd);
        refUScor = strtod(frefDScor, &pEnd);
    ...
    void MCSAnalysis::DoUnfolding(Collection& _USsethold, Collection& _DSsethold, Collection& _USMCsethold, Collection& _DSMCsethold){
        ...
        thetaX_recoGoldhold3 = trkreffix(thetaX_recoGold_noeffi);
        thetaY_recoGoldhold3 = trkreffiy(thetaY_recoGold_noeffi);
