...
//    pStart_vec_y(TVectorD(19)), theta_true_x(TVectorD(19)), theta_true_y(TVectorD(19))
{
  ...
  cor_mom = new TH1D("cor_mom","Cor Momentum Calculated from TOF; Momentum (MeV/c)", 130, 120, 250);
...
void MCSAnalysis::referenceSelection(){
  ...
  for (int i=0; i<Nentries; i++){
    ...
    cuts_accept->Fill("US Track Found",1);
    FillCollectionSciFi(USAllTOFEmpty, jUS, kUS, pz, 0);
    FillCollectionSciFi(DSAllTOFEmpty, jDS, kDS, pz, 1);
    ...
    // if (scifievent->scifitracks().size() != 2) continue;
    // if ( !SelectMomentum() ) continue;
    FillCollectionSciFi(USPreRadSelEmpty, jUS, kUS, pz, 0);
    FillCollectionSciFi(DSPreRadSelEmpty, jDS, kDS, pz, 1);
  if (USrefplaneI > 0 && USabsPlaneI >0 &&
      DSrefplaneI > 0 && DSabsPlaneI >0 ){
...
//	      dthetaY = hy->GetRandom() * _sys["resY"];
  ...
  thetaY_emptyasymm->Fit("pol1","R","",0.,0.02);
  ...
  thetaX_emptyasymm->Fit("pol1","R","",0,0.02);
...
//	    }
  ...
  */
  ...
  thetaY_asymm->Draw();
  thetaY_asymm->Fit("pol1","R","",0,0.02);
  ...
  thetaX_asymm->Draw();
  thetaX_asymm->Fit("pol1","R","",0,0.02);
...
void MCSAnalysis::TruthGraph(Collection& USMCTruthset, Collection& DSMCTruthset, bool recon){
  ...
  MCthetaY_asymm->Draw();
  MCthetaY_asymm->Fit("pol1","R","",0,0.02);
  ...
  MCthetaX_asymm->Draw();
  MCthetaX_asymm->Fit("pol1","R","",0,0.02);
...
void MCSAnalysis::FillVarsVirtual(Vars &tmpvar, int j, int centre){
  ...
  tmpvar.dXdz = mcevent->GetVirtualHits()->at(j).GetMomentum().x()/
    mcevent->GetVirtualHits()->at(centre).GetMomentum().z();
...
void MCSAnalysis::data_make_beam_histograms(Collection Set, std::string desc, std::string suffix, Collection SetMC){
  ...
  tmptitle = desc + ";px residual";
  tmpname  = suffix + "_pxres";
  TH1D* pxres = new TH1D(tmpname.c_str(),tmptitle.c_str(), 100, -10.125, 10.125);
  tmptitle = desc + ";py residual";
  tmpname  = suffix + "_pyres";
  TH1D* pyres = new TH1D(tmpname.c_str(),tmptitle.c_str(), 100, -10.125, 10.125);
  ...
  for(int i=0; i<Set.N(); i++){
    ...
    if (isMC==1){
    pxres->Fill(Set.E(i).px-SetMC.E(i).px);
    pyres->Fill(Set.E(i).py-SetMC.E(i).py);
  ...
  c1->Clear();
  pxres->Draw("colz");
  tmpfile = suffix + "_pxres.pdf";
  c1->Print(tmpfile.c_str());
  c1->Clear();
  pyres->Draw("colz");
  tmpfile = suffix + "_pyres.pdf";
  c1->Print(tmpfile.c_str());
  c1->Clear();
  pzres->Draw("colz");
