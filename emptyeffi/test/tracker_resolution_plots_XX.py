  ...
  for vhit_num in xrange(mc_event.GetVirtualHitsSize()) :
   ...
   if (tof1-tof0) < TOF_ul and (tof1-tof0) > TOF_ll :
    ...
    if station_id == 48 :
      ...
      zdiff = math.fabs(z0 - USZ)
      USX = (USdXdz + sigmap*math.cos(phi)) * zdiff + USX
      USY = (USdYdz + sigmap*math.sin(phi)) * zdiff + USY
      zdi = 13620
      USXdiff = (USdXdz) * zdi + USX
      USYdiff = (USdYdz) * zdi + USY

    if (station_id == 56 and len(w)==3 and len(u)==3 and sqrt(USX*USX + USY*USY)<meanp and sqrt(USXdiff*USXdiff + USYdiff*USYdiff)<100) :
      projTheta = []
...
def calculate_efficiency(plot_dict) :
	...
	pEffx_graph.GetXaxis().SetTitle("#theta_x (mrad)")
	pEffx_graph.Draw("ap")
	...
	pEffy_graph.GetXaxis().SetTitle("#theta_y (mrad)")
	pEffy_graph.Draw("ap")
