...
#     t_BB;
#     t_BB = TOF;
#     dt0 = (12929.260809 - 5287.24720607) / 0.299792458 / 1000.;
#     pzBB = 105.65/sqrt(pow(t_BB,2)/pow(dte01,2)-1);
#     Eini = sqrt(pzBB*pzBB + 105*105);
#     Iair = 85.7e-6;
#     Iscin = 64.7e-6;
#     IAl = 166e-6;
#     IHe = 41.6e-6;
...
#     hwLH2 = 7.64e-6;
#     hwLiH = 18.51e-6;
#     mostprobBBair = MostProbBB(pzBB,Iair,Zair,Aair,hwair,Rair,zair);
#     mostprobBBscin = MostProbBB(pzBB,Iscin,Zscin,Ascin,hwscin,Rscin,zscin);
#     mostprobBBAl = MostProbBB(pzBB,IAl,ZAl,AAl,hwAl,RAl,zAl);
#     mostprobBBHe = MostProbBB(pzBB,IHe,ZHe,AHe,hwHe,RHe,zHe);
#     mostprobBBLH2 = MostProbBB(pzBB,ILH2,ZLH2,ALH2,hwLH2,RLH2,zLH2);
#     mostprobBBLiH = MostProbBB(pzBB,ILiH,ZLiH,ALiH,hwLiH,RLiH,zLiH);
#     BBair = BetheBloch(pzBB,Iair,Zair,Aair,hwair);
#     BBscin = BetheBloch(pzBB,Iscin,Zscin,Ascin,hwscin);
#     BBAl = BetheBloch(pzBB,IAl,ZAl,AAl,hwAl);
#     BBHe = BetheBloch(pzBB,IHe,ZHe,AHe,hwHe);
#     BBLH2 = BetheBloch(pzBB,ILH2,ZLH2,ALH2,hwLH2);
#     ELair = BBair*zair*Rair;
#     ELscin = BBscin*zscin*Rscin;
#     ELAl = BBAl*zAl*RAl;
#     ELHe = BBHe*zHe*RHe;
#     ELLH2 = BBLH2*zLH2*RLH2;
#     pcor;
#     pUS;
#     pDS;
#     if (material=="LiH")i:
#         totEL = mostprobBBair+mostprobBBscin+mostprobBBAl+mostprobBBHe+mostprobBBLiH;
#         Efin = Eini - totEL;
#         if (isdata):
...
def get_expected_tracks(mc_event, virtual_plane_dict, scifi_event, tofevent) :
  ...
  for vhit_num in xrange(mc_event.GetVirtualHitsSize()) :
   ...
   # if (MomCalc(tof1-tof0) < P_ul and MomCalc(tof1-tof0) > P_ll :
   if (tof1-tof0) < TOF_ul and (tof1-tof0) > TOF_ll :
    ...
    if (station_id == 73 and len(w)==3 and len(u)==3 and sqrt(USXproj*USXproj + USYproj*USYproj)<meanp and sqrt(USXdiff*USXdiff + USYdiff*USYdiff)<90) :
      projTheta = []
