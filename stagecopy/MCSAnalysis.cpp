...
//    pStart_vec_y(TVectorD(19)), theta_true_x(TVectorD(19)), theta_true_y(TVectorD(19))
{
  ...
  */
  ...
  refchain   = new TTree();
  mcchain = new TTree();
  mcemptychain = new TChain();
  */
  /*
  TTree* chain   = new TTree();
  TTree* refchain   = new TTree();
  TTree* mcchain = new TTree();
  TTree* mcemptychain = new TChain();
  */
  ...
  chi2pern = new TH1D("chi2n","chi2n", 500, 0, 500);
...
void MCSAnalysis::StageData(TString treename, TString fn){

  TFile* f= new TFile(fn);
  TTree* tree=(TTree*)f->Get(treename);
  dataSelection(mode, tree);
  delete tree;
  f->Close();

}
	
void MCSAnalysis::StageRefData(TString treename, TString fn){

  TFile* f= new TFile(fn);
  TTree* reftree=(TTree*)f->Get(treename);
  referenceSelection(reftree);
  //reftree->Delete();
  delete reftree;
  f->Close();
  std::cout << "time to print" << std::endl;
  gDirectory->Print();

}

void MCSAnalysis::StageMCData(TString treename, TString fn){

  TFile* f= new TFile(fn);
  TTree* mctree=(TTree*)f->Get(treename);
  TruthData(mode, mctree);
  delete mctree;
  f->Close();
...
void MCSAnalysis::Execute(int mode){
  ...
  //PlotRunInfo();
  make_beam_histograms(USAllTOFEmpty, "Upstream, Data Reference", "dataUSref_alltof");
  make_beam_histograms(DSAllTOFEmpty, "Downstream, Data Reference", "dataDSref_alltof");
  //make_acceptance_histograms(USAllTOF, DSAllTOF, "Data Reference", "dataProj_alltof");
  make_beam_histograms(USPreRadSelEmpty, "Upstream, Data Reference", "dataUSref_prerad");
  make_beam_histograms(DSPreRadSelEmpty, "Downstream, Data Reference", "dataDSref_prerad");
...
void MCSAnalysis::dataSelection(int mode, TTree* tree){
  // Set addresses for tree selection
  tree->SetBranchAddress("RunNumber", &runnumber);
  tree->SetBranchAddress("TOFBranch", &tofevent);
  tree->SetBranchAddress("SciFiBranch", &scifievent);
  tree->SetBranchAddress("CkovBranch", &ckovevent);
  tree->SetBranchAddress("KLBranch", &klevent);
  tree->SetBranchAddress("EMRBranch", &emrevent);
...
void MCSAnalysis::referenceSelection(TTree* reftree){
  // Set addresses for tree selection
  reftree->SetBranchAddress("TOFBranch", &tofevent);
  reftree->SetBranchAddress("SciFiBranch", &scifievent);
  reftree->SetBranchAddress("CkovBranch", &ckovevent);
  reftree->SetBranchAddress("KLBranch", &klevent);
  reftree->SetBranchAddress("EMRBranch", &emrevent);
  reftree->SetBranchAddress("MCEvent", &mcevent);

  /*
  TBranch* branch = reftree->GetBranch("TOFBranch");
  branch->SetAutoDelete(kTRUE);
  TBranch* branchsci = reftree->GetBranch("SciFiBranch");
  branchsci->SetAutoDelete(kTRUE);
  TBranch* branchckov = reftree->GetBranch("CkovBranch");
  branchckov->SetAutoDelete(kTRUE);
  TBranch* branchkl = reftree->GetBranch("KLBranch");
  branchkl->SetAutoDelete(kTRUE);
  TBranch* branchemr = reftree->GetBranch("EMRBranch");
  branchemr->SetAutoDelete(kTRUE);
  TBranch* branchmc = reftree->GetBranch("MCEvent");
  branchmc->SetAutoDelete(kTRUE);
  */
  // Restrict the number of entries to less than or equal to the refchain entries
  
  int Nentries = reftree->GetEntries();
  // < mcchain->GetEntries() ? 
  //  chain->GetEntries() : mcchain->GetEntries();
...
void MCSAnalysis::generateMCSResponse(TTree* mctree){

  mctree->SetBranchAddress("TOFBranch", &tofevent);
  mctree->SetBranchAddress("SciFiBranch", &scifievent);
  mctree->SetBranchAddress("CkovBranch", &ckovevent);
  mctree->SetBranchAddress("KLBranch", &klevent);
  mctree->SetBranchAddress("EMRBranch", &emrevent);
  mctree->SetBranchAddress("MCEvent", &mcevent);
...
void MCSAnalysis::TruthData(int mode, TTree* mctree){
...
//	    std::cout << "USTruthSet.E(USTruthSet.N()-1).px " << USTruthSet.E(USTruthSet.N()-1).px << std::endl;
  ...
  if (mode == 3) {
	  TruthGraph(USTruthSetLiH, DSTruthSetLiH);
  }
  if (mode == 4) {
  }
  /*
...
void MCSAnalysis::FillCrossCheck(){
...
void MCSAnalysis::FillVarsSciFi(Vars& tmpvar, int j, int k, double pz, int isDS){
  ...
  if(j < int(scifievent->scifitracks().size()) && j != -1){
    if(k < int(scifievent->scifitracks()[j]->scifitrackpoints().size()) && k != -1){
      ...
      if(isEmpty){
      if(isDS==0){
	      tmpvar.dXdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x()*cos(rot_ang_empty)+sin(rot_ang_empty) + tan(thX*atan(1.)/45.0);
      } else { 
	      tmpvar.dXdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x() + tan(thX*atan(1.)/45.0);
      }
      ...
      if(isDS==0){
	      ...
	      tmpvar.dXdz = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x()*cos(rot_ang) + sin(rot_ang)+tan(thX*atan(1.)/45.0);
	      double ori = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x() + tan(thX*atan(1.)/45.0);
	      double justcos = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x()*cos(rot_ang) + tan(thX*atan(1.)/45.0);
	      double justsin = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x()*cos(rot_ang)+sin(rot_ang) + tan(thX*atan(1.)/45.0);
	      double full = scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().x()*cos(rot_ang)+scifievent->scifitracks()[j]->scifitrackpoints()[k]->gradient().z()*sin(rot_ang) + tan(thX*atan(1.)/45.0);
	      /*
