...
// source_* changed each iteration so Source_* is left unchanged
  ...
  for(int i=1; i<=itMax; i++){
    ...
    sX->DeconvolutionRL(source_thetaX, response_thetaX, sp+gap+int(_histlimits["NbinsXY"]), 100, 20, 0);
    sY->DeconvolutionRL(source_thetaY, response_thetaY, sp+gap+int(_histlimits["NbinsXY"]), 100, 20, 0);
...
// test results added to arrays
  ...
  for(int i=1; i<=itMax; i++){
    ...
    sScatt->DeconvolutionRL(source_thetaScatt, response_thetaScatt, 2*(gap+sp)+int(_histlimits["NbinsTh"]), 100, 20, 0);
  ...
  for(int i=1; i<=itMax; i++){
    ...
    sScatt2->DeconvolutionRL(source_theta2Scatt, response_theta2Scatt, sp+gap+int(_histlimits["NbinsTh2"]), 100, 20, 0);
