...
void evencloserpeaks(Int_t np=1) {
   Double_t *x = 0;
   ...
   for (i = 1; i < nbins; i++) hdeconpic->SetBinContent(i,hdecon->GetBinContent(i+300));
   ...
   hevendeconpic->Draw("SAME");
   hevendeconpic->SetLineColor(kRed);
   c1->SetLogy();
   float residual = 0;
   ...
   //mirrorstat->Draw("AP"); 
   c1->Print("Goldevenexample.pdf");
   c3->Print("c3Goldevenexample.pdf");
}
