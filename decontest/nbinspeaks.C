...
void nbinspeaks(Int_t np=1) {
   //Double_t *x = 0;
   TGraph *mirrorstat = new TGraph();
	   int zz = 0;

   const Int_t nbins = 1000000;
   npeaks = TMath::Abs(np);
   ...
   c1->Divide(2,2);
   c1->cd(1);
   h->FillRandom("f",20000000);
   h->Draw();
   h->Fit("gaus");

   TH1F *h2 = new TH1F("h","Empty",nbins,0,1000);
   TH1F *h5 = new TH1F("h","Empty",nbins,0,1000);
   Double_t par[3000];
   Int_t p;
   par[3*0+2] = 100;
   par[3*0+3] = 100;
   ...
   */
   ...
   for (int jj=10; jj<10001; jj=jj*10){ 
   TH1F *hdecon = new TH1F("h","Gold Decon.",nbins/jj,0,1000);
   TH1F *hnewcon = dynamic_cast<TH1F*>(hcon->Rebin(jj,"hnewcon"));
   c1->cd(3);
   hnewcon->Draw();
   TH1F *hnew2 = dynamic_cast<TH1F*>(h2->Rebin(jj,"hnew2"));
   c1->cd(2);
   hnew2->Draw();
   Int_t i;
   Double_t xmin     = 0;
   Double_t xmax     = 2*nbins;
   float* Source_thetaX = new float[nbins/jj];
   float* Response_thetaX = new float[nbins/jj];
   //gROOT->ForceStyle();
   //for (i = 0; i < 100; i++) Source_thetaX[i]=0;
   for (i = 0; i < nbins/jj; i++) Source_thetaX[i]=hnewcon->GetBinContent(i + 1);
   for (i = 0; i < nbins/jj; i++) Response_thetaX[i]=hnew2->GetBinContent(i + 1);
   //for (i = 100; i < nbins; i++) Response_thetaX[i]=0;
   TSpectrum *s = new TSpectrum();
   s->Deconvolution(Source_thetaX,Response_thetaX,nbins/jj,10,1,0);
   for (i = 0; i < nbins/jj; i++) hdecon->SetBinContent(i + 1,Source_thetaX[i]);
   //TH1F *hnewdecon = dynamic_cast<TH1F*>(hdecon->Rebin(jj,"hnew2"));
   //hdecon->Rebin(2);
   ...
   zz += 1;
   Double_t residual = hdecon->GetMean()-h->GetMean();
   residual = sqrt(pow(residual,2));
   mirrorstat->SetPoint(zz,nbins/jj,residual);
   }
