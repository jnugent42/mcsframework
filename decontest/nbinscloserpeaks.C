...
void nbinscloserpeaks(Int_t np=1) {
   Double_t *x = 0;
   TGraph *mirrorstat = new TGraph();
	   int zz = 0;
   for (int jj=100; jj<10000; jj=jj+10){ 
	   std::cout << jj << std::endl;
   npeaks = TMath::Abs(np);
   const Int_t nbins = jj;
   TH1F *h = new TH1F("h","Abs.",jj,0,1000);
   Int_t p;
   Double_t norm  = 100;
   Double_t mean  = 600;
   Double_t sigma = 30+2;
   ...
   h->Scale(1./hn);
   h->Draw();
   TH1F *hpic = new TH1F("hpic","Abs.",jj/5,-100,100);
   for (int i = 1; i < nbins; i++) hpic->SetBinContent(i,h->GetBinContent(i+jj250));
   hpic->Draw();
   int mirronbins = 100;
