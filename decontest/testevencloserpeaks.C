...
void testevencloserpeaks(Int_t np=1) {
   Double_t *x = 0;
   ...
   //for (i = 1; i < mirronbins/2+1; i++) hpicl->SetBinContent(i,hpic->GetBinContent(i));
   //for (i = 1; i < mirronbins/2+1; i++) hpicr->SetBinContent(i,hpic->GetBinContent(mirronbins+1-i));
   hpic->Draw();
   int mirronbins = 100;
   ...
   TFile* rf = new TFile("../scanradial/accp_rad_0/LiHMu_3200_140.root","READ");
   TH1F* h2 = rf->Get("thetaX_ref");
   c1->cd(2);
   gPad->SetLogy();
   h2n = h2.GetEntries();
   h2->Scale(1./h2n);
   h2->Draw();
   TH1F *h2pic = new TH1F("hpic","Empty",46,-0.06,0.06);
   ...
   TH1F* hcon = rf->Get("thetaX_data");
   c1->cd(3);
   gPad->SetLogy();
   hconn = hcon->GetEntries();
   hcon->Scale(1./hconn);
   hcon->Draw();
   TH1F *hconpic = new TH1F("hconpic","Convolution",46,-0.06,0.06);
   for (i = 1; i < nbins; i++) hconpic->SetBinContent(i,hcon->GetBinContent(i));
   hconpic->Draw();
   ...
   TH1F *hdecon = new TH1F("h","Gold Decon.",46,-0.06,0.06);
   Int_t i;
   Double_t xmin     = 0;
   ...
   //gROOT->ForceStyle();
   for (i = 0; i < nbins; i++) Source_thetaX[i]=0;
   for (i = 100; i < 146; i++) Source_thetaX[i]=hconpic->GetBinContent(i+1-100);
   for (i = 0; i < 46; i++) Response_thetaX[i]=h2pic->GetBinContent(i + 1);
   for (i = 47; i < nbins; i++) Response_thetaX[i]=0;
   TSpectrum *s = new TSpectrum();
   s->DeconvolutionRL(Source_thetaX,Response_thetaX,500,10,1,0);
   for (i = 0; i < nbins; i++) hdecon->SetBinContent(i + 1,Source_thetaX[i+100]);
   ...
   hdecon->Draw();
   TH1F *hdeconpic = new TH1F("hdeconpic","Gold Decon.",100,-100,100);
   TH1F *hevendeconpic = new TH1F("hevendeconpic","Gold Decon.",100,-100,100);
   for (i = 1; i < 47; i++) hdeconpic->SetBinContent(i,hdecon->GetBinContent(i+47));
   for (i = 1; i < mirronbins+1; i++) hevendeconpic->SetBinContent(i,(hdeconpic->GetBinContent(i)+hdeconpic->GetBinContent(mirronbins+1-i))/2);
   //hpic->SetStats(false);
   //hpic->Draw();
   hevendeconpic->SetStats(true);
   //hdeconpic->Draw();
   ...
   //for (i = 1; i < mirronbins/2+1; i++) hdeconmirrorl->SetBinContent(i,hevendeconpic->GetBinContent(i));
   //for (i = 1; i < mirronbins/2+1; i++) hdeconmirrorr->SetBinContent(i,hevendeconpic->GetBinContent(mirronbins+1-i));
