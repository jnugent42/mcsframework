...
p1                        =      1.17466   +/-   0.00142602
    ...
    for graph in graphs:
        ...
        if graph.GetName() == "tofrefmomentum":
            ...
            leg.AddEntry(ftof, "Fit to #frac{m}{p} + c (m="+str(round(ftof.GetParameter(0),2))+" c="+str(round(ftof.GetParameter(1),2))+")", "l")
        if graph.GetName() == "truthmomentum":
            graph.Fit('ftof')
            print "Truth TOF for 172 MeV/c is ",graph.GetFunction('ftof').Eval(172)
            print "Truth TOF for 200 MeV/c is ",graph.GetFunction('ftof').Eval(200)
            print "Truth TOF for 240 MeV/c is ",graph.GetFunction('ftof').Eval(240)
            leg.AddEntry(ftof, "Fit to #frac{m}{p} + c (m="+str(round(ftof.GetParameter(0),2))+" c="+str(round(ftof.GetParameter(1),2))+")", "l")
        if graph.GetName() == "reftruthmomentum":
            graph.Fit('ftof')
            print "Truth ref TOF for 172 MeV/c is ",graph.GetFunction('ftof').Eval(172)
            print "Truth ref TOF for 200 MeV/c is ",graph.GetFunction('ftof').Eval(200)
            print "Truth ref TOF for 240 MeV/c is ",graph.GetFunction('ftof').Eval(240)
            leg.AddEntry(ftof, "Fit to #frac{m}{p} + c (m="+str(round(ftof.GetParameter(0),2))+" c="+str(round(ftof.GetParameter(1),2))+")", "l")
		...
		#print (float(point[1][0]) - float(point[0][0]))/2.
            ...
            if graphs[i].GetName() == "tofrefmomentum" and float(point[17][0]) > 10:
                graphs[i].SetPoint(j, (scale * float(point[17][0]) + offset),
                                   float(point[i][0]))
            if graphs[i].GetName() == "truthmomentum" and float(point[20][0]) > 10:
	        print float(point[16][0])
                graphs[i].SetPoint(j, (scale * float(point[20][0]) + offset),
                                   (float(point[1][0]) + float(point[0][0]))/2.)
            if graphs[i].GetName() == "reftruthmomentum" and float(point[21][0]) > 10:
